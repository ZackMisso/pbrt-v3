...
TextureDensityFunction::TextureDensityFunction(Spectrum scale, std::shared_ptr<Texture<Spectrum>> texture)
    : scale(scale), Density(texture) { }

Spectrum TextureDensityFunction::D(const Point3f& p) const
{
    nDensityCalls++;
